...
public class PileRecordController extends BaseController{


    private Map<Long, String>  pileIdAndName;

    public void listPage() {
/*        List<ChargePile> chargePiles = ChargePile.dao.find("select * from yc_charge_pile ");
        pileIdAndName = new HashMap<Long, String>();
        Map<String,Long>  pileNameAndId=new HashMap<>();
        for (ChargePile chargePile:chargePiles){
            pileIdAndName.put(chargePile.getId(),chargePile.getName());
            pileNameAndId.put(chargePile.getName(),chargePile.getId());
        }
        setAttr("pileNameAndId", pileNameAndId);*/
    ...
    public void listData() {
        ...
        List<Record> list = DBTool.findByMultPropertiesDbSource("zcurd_busi", "yc_charge_pile_history", properties, symbols, values, orderBy, this.getPager());
/*        for (Record record:list){
            record.set("pile_name",pileIdAndName.get(record.get("pile_id")));
        }*/
        this.renderDatagrid(list, DBTool.countByMultPropertiesDbSource("zcurd_busi", "yc_charge_pile_history", properties, symbols, values));
    }
