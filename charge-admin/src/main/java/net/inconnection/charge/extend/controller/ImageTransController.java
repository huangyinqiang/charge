...
import net.inconnection.charge.extend.chargeDevice.jms.ImageTransMQClient;
...
public class ImageTransController extends BaseController {

    private static Logger _log = LoggerFactory.getLogger(ImageTransController.class);
    ...
    public void uploadImage() {
        ...
        String fileRedisKey = getPara("fileName");
    ...
    public void transImageProcess() {
        ...
        ImageTransMQClient imageTransMQClient = new ImageTransMQClient();
        try {
            imageTransMQClient.start();
            _log.info("JMSCorrelationID  "+key);
            imageTransMQClient.requestStartTransImage(key,json.toJSONString());
        } catch (Exception e) {
            _log.error("DeviceUpdateMQClient 客户端启动错误!",e);
        }
        //阻塞并从队列等待反馈
        String status = imageTransMQClient.getUpdateResult(TIME_OUT);
        try {
            imageTransMQClient.stop();
        } catch (Exception e) {
