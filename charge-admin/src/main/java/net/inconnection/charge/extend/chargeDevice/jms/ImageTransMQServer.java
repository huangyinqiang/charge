...
public class ImageTransMQServer implements MessageListener{

    private ThreadPoolTaskExecutor threadPoolTaskExecutor;

    private static Log _log = Log.getLog(ImageTransMQServer.class);
    ...
    //单例模式
    private static final ImageTransMQServer instance = new ImageTransMQServer();



    //私有化构造方法
    private ImageTransMQServer(){
        start();
    }

    //单例方法
    public static ImageTransMQServer getInstance(){
    ...
    private void setupConsumer() throws JMSException {
        ...
        Destination deviceUpdateQueue = session.createQueue(IMAGE_QUEUE);
...
//        threadPoolTaskExecutor.execute(() -> {
            try {
                if (message instanceof TextMessage) {
                    ...
                    _log.info("收到图像请求消息:"+txtMsg.getJMSCorrelationID());
                    _log.info("图像消息内容为:"+messageText);
                    ...
                    ImageMsgHandle.sendFirstFirmwareDataFrame(instance,txtMsg.getJMSCorrelationID(),jsonObject);
                }
            } catch (JMSException e) {
