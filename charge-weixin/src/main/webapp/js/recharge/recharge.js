...
//		$('#radioCH')
	...
	$('.elecard-wallte').on('click', '.weui-flex__item', function() {
		...
		selectedHour = $(this).children().html();// 充值金额
...
function rechargeAmount() {
	...
	if (selectedtype == 'WA') {
		...
		type = "CH";// 充值类型
		// 如果选择电卡，绑定后才可以充值
		if (userInfo.band == 'N' || userInfo.cardNumber == null
				|| userInfo.cardNumber == '') {
		...
		for ( var key in activityInfo) {
			var act = activityInfo[key];
			if ('1001' == act.actNum) {// 活动编号=选中的金额编号
				$(".weui-flex__item a[data-date='1001']").text(
						act.money / 100 + "元")
			}
			if ('1002' == act.actNum) {// 活动编号=选中的金额编号
				$(".weui-flex__item a[data-date='1002']").text(
						act.money / 100 + "元")
			}
			if ('1003' == act.actNum) {// 活动编号=选中的金额编号
				$(".weui-flex__item a[data-date='1003']").text(
						act.money / 100 + "元")
			}
			if ('1005' == act.actNum) {// 活动编号=选中的金额编号
				$(".weui-flex__item a[data-date='1005']").text(
						act.money / 100 + "元")
			}
			if ('1006' == act.actNum) {// 活动编号=选中的金额编号
				$(".weui-flex__item a[data-date='1006']").text(
						act.money / 100 + "元")
			}
		}
	}
...
function getActivityInfo() {
	$.ajax({
		type : 'POST',
		url : "../recharge/queryActivityInfo",
		dataType : "json",
		data : {
